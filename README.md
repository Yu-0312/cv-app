...
- **Career 求職顧問**：自動讀取 CV 履歷頁摘要，分析職缺適配度、推薦崗位、產生面試準備與求職信；目前不提供 PDF / CV 檔案上傳。
...
此功能採用 `career-ops` 類型的評估思路（A-F / 分數、ATS 關鍵字、STAR 面試故事），但不是完整移植 GitHub 上 `santifer/career-ops` 的 CLI、批次職缺 pipeline、追蹤器或客製 ATS PDF 產生流程。
